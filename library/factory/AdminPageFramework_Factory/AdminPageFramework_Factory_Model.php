 ...
 */
abstract class CampaignMonitor_AdminPageFramework_Factory_Model extends CampaignMonitor_AdminPageFramework_Factory_Router {
    ...
    protected function _loadFieldTypeDefinitions() {
        if (empty(self::$_aFieldTypeDefinitions)) {
            self::$_aFieldTypeDefinitions = CampaignMonitor_AdminPageFramework_FieldTypeRegistration::register(array(), $this->oProp->sClassName, $this->oMsg);
        }
    ...
    protected function _registerField(array $aField) {
        CampaignMonitor_AdminPageFramework_FieldTypeRegistration::_setFieldResources($aField, $this->oProp, $this->oResource);
        if ($aField['help']) {
    ...
    protected function _isValidationErrors() {
        if (isset($GLOBALS['aCampaignMonitor_AdminPageFramework']['aFieldErrors']) && $GLOBALS['aCampaignMonitor_AdminPageFramework']['aFieldErrors']) {
            return true;
        }
    ...
    public function _replyToSaveFieldErrors() {
        if (!isset($GLOBALS['aCampaignMonitor_AdminPageFramework']['aFieldErrors'])) {
            return;
        }
        $this->oUtil->setTransient("apf_field_erros_" . get_current_user_id(), $GLOBALS['aCampaignMonitor_AdminPageFramework']['aFieldErrors'], 300);
    }
    public function _replyToSaveNotices() {
        if (!isset($GLOBALS['aCampaignMonitor_AdminPageFramework']['aNotices'])) {
            return;
        }
        if (empty($GLOBALS['aCampaignMonitor_AdminPageFramework']['aNotices'])) {
            return;
        }
        $this->oUtil->setTransient('apf_notices_' . get_current_user_id(), $GLOBALS['aCampaignMonitor_AdminPageFramework']['aNotices']);
    }
