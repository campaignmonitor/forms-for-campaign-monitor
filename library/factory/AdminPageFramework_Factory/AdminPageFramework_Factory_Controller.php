 ...
 */
abstract class CampaignMonitor_AdminPageFramework_Factory_Controller extends CampaignMonitor_AdminPageFramework_Factory_View {
    ...
    public function setFieldErrors($aErrors) {
        $GLOBALS['aCampaignMonitor_AdminPageFramework']['aFieldErrors'] = $this->oUtil->getElement($GLOBALS, array('aCampaignMonitor_AdminPageFramework', 'aFieldErrors'), array());
        if (empty($GLOBALS['aCampaignMonitor_AdminPageFramework']['aFieldErrors'])) {
            add_action('shutdown', array($this, '_replyToSaveFieldErrors'));
        }
        $_sID = md5($this->oProp->sClassName);
        $GLOBALS['aCampaignMonitor_AdminPageFramework']['aFieldErrors'][$_sID] = isset($GLOBALS['aCampaignMonitor_AdminPageFramework']['aFieldErrors'][$_sID]) ? $this->oUtil->uniteArrays($GLOBALS['aCampaignMonitor_AdminPageFramework']['aFieldErrors'][$_sID], $aErrors) : $aErrors;
    }
    public function hasFieldError() {
        return isset($GLOBALS['aCampaignMonitor_AdminPageFramework']['aFieldErrors'][md5($this->oProp->sClassName) ]);
    }
    public function setSettingNotice($sMessage, $sType = 'error', $asAttributes = array(), $bOverride = true) {
        $GLOBALS['aCampaignMonitor_AdminPageFramework']['aNotices'] = $this->oUtil->getElement($GLOBALS, array('aCampaignMonitor_AdminPageFramework', 'aNotices'), array());
        if (empty($GLOBALS['aCampaignMonitor_AdminPageFramework']['aNotices'])) {
            add_action('shutdown', array($this, '_replyToSaveNotices'));
        }
        $_sID = md5(trim($sMessage));
        if ($bOverride || !isset($GLOBALS['aCampaignMonitor_AdminPageFramework']['aNotices'][$_sID])) {
            ...
            $GLOBALS['aCampaignMonitor_AdminPageFramework']['aNotices'][$_sID] = array('sMessage' => $sMessage, 'aAttributes' => $_aAttributes + array('class' => $sType, 'id' => $this->oProp->sClassName . '_' . $_sID,),);
        }
    }
    public function hasSettingNotice($sType = '') {
        $_aNotices = $this->oUtil->getElementAsArray($GLOBALS, array('aCampaignMonitor_AdminPageFramework', 'aNotices'), array());
        if (!$sType) {
